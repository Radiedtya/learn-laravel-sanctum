...
onMounted(async () => {
  ...
  try {
    const [filmRes, gRes, aRes] = await Promise.all([
      api.get(`/films/${route.params.id}`),
    ...
    form.slug = film.slug || "";
    form.genre_id = film.genre_id || "";
    form.sutradara = film.sutradara || "";
    form.tahun_rilis = film.tahun_rilis || "";
    form.durasi = film.durasi || "";
    form.rating = film.rating || "";
    form.poster = film.poster || "";
    form.deskripsi = film.deskripsi || "";

    // Aktor yang sudah terhubung ke film ini
    if (film.aktors && film.aktors.length > 0) {
      form.aktor_ids = film.aktors.map((a) => a.id);
    }
  } catch (err) {
    if (err.response?.status === 404) {
      errorMsg.value = "Film tidak ditemukan.";
    } else {
...
const handleSubmit = async () => {
  ...
  try {
    await api.put(`/films/${route.params.id}`, form);
    successMsg.value = "Film berhasil diperbarui.";
    ...
    if (err.response?.status === 422) {
      ...
      errorMsg.value = err.response?.data?.message || "Gagal memperbarui film.";
    }
  } finally {
    saving.value = false;
  }
      ...
      <form @submit.prevent="handleSubmit" class="form-card">
        <div class="form-row">
          <div class="field">
            ...
            <input id="judul" v-model="form.judul_film" type="text" required />
          </div>
          <div class="field">
        ...
        <div class="field">
          ...
          <input id="poster" v-model="form.poster" type="url" required />
        ...
        <div class="field">
          ...
          <textarea id="deskripsi" v-model="form.deskripsi" rows="5"></textarea>
        </div>
        ...
        <div class="form-actions">
          <RouterLink to="/kelola-film" class="btn-cancel">Batal</RouterLink>
